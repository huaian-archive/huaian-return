    ...
    <main>
      ...
      <section className="persons" id="persons">
        ...
        <div className="person-layout"><div className="person-list" role="tablist" aria-label="八人名冊">{people.map((person, i) => <button key={person.no} role="tab" aria-selected={i === active} onClick={() => setActive(i)}><em>{person.no}</em><span>{person.name}<small>{person.role}</small></span><i>{person.key}</i></button>)}</div>
          <article className="dossier" aria-live="polite"><div className="dossier-top"><span>第 {p.no} 項</span><span>STATUS / 在籍</span></div><div className="dossier-content"><div className="portrait-wrap">{p.image ? <img className="portrait" src={p.image} alt={`${p.name}角色照片`} /> : <div className="portrait-missing"><b>01</b><span>影像缺件</span></div>}<q>{p.quote}</q></div><div className="record-copy"><p className="filelabel">PERSONNEL RECORD</p><h3>{p.name}</h3><p className="role">{p.role} · {p.age} 歲</p><div className="redline" /><dl><div><dt>識別特徵</dt><dd>{p.mark}</dd></div><div><dt>核心判斷</dt><dd>{p.core}</dd></div><div><dt>最後察覺</dt><dd>第 {p.no} 項成立時，確認八人的事在轉移給「你」。</dd></div></dl></div></div><div className="stamp">已核<br />在籍</div></article></div>
      </section>
